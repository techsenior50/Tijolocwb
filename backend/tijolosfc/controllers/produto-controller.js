...
class ProdutoController {
    ...
    adicionar(req, res) {
        ...
        let produto = new Produto(req.body.categoriaProduto,
                                  req.body.nomeProduto, req.body.descricaoProduto,
                                  req.body.precoProduto, ativoProduto, req.body.imagemProduto,
                                  req.requestTime) //para que o nodejs entenda o body do json preciso inicializar antes com o comando app.use(express.json())
    ...
    async buscarTodosPaginado(req, res) {
        ...
        try {
            const produtos = await this.produtoService.buscarTodosPaginado(page,
                                                                           limit) //preciso colocar this.produtoService porque é uma propriedade, mas esse this pode dar conflito, pois ele varia de acordo com o escopo. Quando um método get chama essa função ele pode entender que esse this é de quem chamou e não é do produtoController. Para que não ocorra essa confusão e dê erro "Cannot read property 'produtoService' of undefined, quando chamo a função, preciso passar de que this se trata especificamente que no caso é do produtoController"
            const totalItens = Object.keys(produtos).length
            res.render('listarTodosPaginado', {
                produtos, totalPages: Math.ceil(totalItens / limit),
                currentPage: page,
            })
        } catch (err) {
